// app/stores/[storeName]/page.js - Fixed version
"use client"
...
export default function StorePage() {
  ...
  const fetchStoreData = async (name) => {
    setLoading(true);
    try {
      console.log(`Fetching store data for: ${name}`);
      const response = await fetch(`/api/stores/${name}`);
  ...
  // If there's an error, we show error page
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-md max-w-md w-full">
          ...
          <p className="text-gray-600 mb-6">
            {error}. The store you are looking for does not exist or may have been moved.
  ...
  return (
    <>
      <Head>
        <title>{store ? `${store.name}'s Store` : 'Store'} | QuoteFlow</title>
        ...
        <header className="bg-white shadow">
          <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            <h1 className="text-3xl font-bold text-gray-900">
              {store ? `${store.name}'s Store` : 'Loading...'}
            </h1>
